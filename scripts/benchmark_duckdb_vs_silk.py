...
COPY (
    SELECT
        ...
        ((i % 1000000000) + 1000000000)::BIGINT as npi,
        (((i * 37) % 1000000000) + 1000000000)::BIGINT as target_npi,
...
def run_silk_conversion(
    ...
    warmup_cmd = [
        *silk_cmd,
        ...
        "v2",
        "--parquet-column-encoding-threads",
        str(threads),
        "--overwrite",
    ]
    start = time.perf_counter()
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=34816,
        help="Memory limit in MiB for DuckDB (default: 34816)",
    )
    parser.add_argument(
        "--rows",
        type=int,
    ...
    silk_result = run_silk_conversion(silk_cmd, arrow_file, silk_output, args.threads)
