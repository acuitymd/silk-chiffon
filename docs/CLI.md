...
- `--from <FROM>` - Single input as a local path, file:// URL, or gs:// URI
- `--from-many <FROM_MANY>` - Inputs as local paths, file:// URLs, or gs:// URIs. Supports quoted glob patterns and may be repeated
...
- `--to <TO>` - Single output as a local path, file:// URL, or gs:// URI
- `--to-many <TO_MANY>` - Local or gs:// output template (e.g., "{{region}}.parquet"). Quote the template in the shell. Requires --by
...
- `--spill-path <SPILL_PATH>` - Local directory only. gs:// is not supported for spilling intermediate data.
...
- `--list-outputs-file <LIST_OUTPUTS_FILE>` - Write the output listing to a local path or gs:// URI instead of stdout. Remote manifests commit after data outputs
- `--create-dirs` - Create local parent directories as needed. GCS prefixes need no directory creation

  Default value: `true`
- `--overwrite` - Overwrite existing data and manifest objects. The default is race-safe create-only commit
...
- `<FILE>` - Local path, file URL, or GCS location to identify
...
- `<FILE>` - Local path, file URL, or GCS location of the Parquet file
...
- `<FILE>` - Local path, file URL, or GCS location of the Arrow IPC file
...
- `--row-count` - Count rows (Arrow streams are read sequentially)
...
- `<FILE>` - Local path, file URL, or GCS location of the Vortex file
