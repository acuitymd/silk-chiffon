...
use crate::{
    ...
    sinks::{
        ...
        data_sink::DataSink,
        parquet::{ParquetSink, ParquetSinkOptions, ParquetThreadPools},
...
pub async fn run(args: TransformCommand) -> Result<()> {
    let TransformCommand {
        ...
        parquet_compression,
        parquet_ingest_queue_size,
        parquet_encoding_queue_size,
        parquet_write_queue_size,
        parquet_encoding,
        parquet_io_threads,
    ...
    let cpus = std::thread::available_parallelism()
        .map(|p| p.get())
        .unwrap_or(4);
    let pools = Arc::new(ParquetThreadPools::new(
...
fn create_sink_factory(
    ...
    parquet_ingest_queue_size: usize,
    parquet_encoding_queue_size: usize,
    parquet_write_queue_size: usize,
    ...
    Ok(Box::new(move |path: String, schema: SchemaRef| {
        ...
        let sink: Box<dyn DataSink> = match detected_format {
            ...
            DataFormat::Parquet => {
                ...
                options = options
                    .with_ingest_queue_size(parquet_ingest_queue_size)
                    .with_encoding_queue_size(parquet_encoding_queue_size)
                    .with_write_queue_size(parquet_write_queue_size);
