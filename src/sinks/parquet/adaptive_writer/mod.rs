...
//! # Architecture
//!
//! The writer uses a three-task pipeline for high throughput:
//!
//! ```text
//! [Ingestion Task] → [Encoder Tasks (N)] → [Writer Task]
//!       ↓
//! [Analysis (optional)]
//! ```
//!
...
use crate::sinks::parquet::{ParquetThreadPools, ParquetWriter};
use crate::utils::cancellable_channel::{CancellableSender, cancellable_channel_bounded};
use crate::utils::first_error::FirstError;
use pipeline::run_pipeline;

pub struct AdaptiveParquetWriter {
    batch_sender: Option<CancellableSender<RecordBatch>>,
...
impl AdaptiveParquetWriter {
    pub fn new(
        ...
        let (batch_tx, batch_rx) = cancellable_channel_bounded::<RecordBatch>(
    ...
    pub async fn write(&mut self, batch: RecordBatch) -> Result<()> {
        ...
        sender.send(batch).await.map_err(|_| {
            self.first_error
                .take()
                .unwrap_or_else(|| anyhow!("batch send failed: writer pipeline closed"))
        })
    }
    ...
    pub async fn close(mut self) -> Result<u64> {
        ...
        match handle.await {
            Ok(Ok(rows)) => Ok(rows),
            Ok(Err(e)) => Err(self.first_error.take().unwrap_or(e)),
    ...
    pub async fn cancel(mut self) -> Result<()> {
        self.first_error.cancel_token().cancel();
        self.batch_sender.take();
...
impl Drop for AdaptiveParquetWriter {
    fn drop(&mut self) {
        if self.monitor_handle.take().is_some() {
            self.first_error.cancel_token().cancel();
        }
    }
}
...
mod tests {
    ...
    async fn test_adaptive_writer_basic() {
        ...
        let mut writer =
            AdaptiveParquetWriter::new(&output_path, &schema, props, test_pools(), config);
