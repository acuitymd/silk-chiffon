...
pub fn build_row_group_properties(
    ...
    let mut builder = WriterProperties::builder()
        ...
        .set_offset_index_disabled(base_props.offset_index_disabled())
        .set_coerce_types(base_props.coerce_types())
    ...
    for leaf in enumerate_leaf_columns(schema) {
        ...
        let col_path = ColumnPath::from(leaf.path);
...
/// Check if a data type can be analyzed for cardinality.
///
/// Returns false for nested types (List, Struct, Map) which can't be analyzed,
/// and floats which rarely benefit from analysis due to high cardinality.
pub fn is_analyzable(data_type: &DataType) -> bool {
    !matches!(
        data_type,
        DataType::Float16
            | DataType::Float32
...
mod tests {
    ...
    #[test]
    fn test_build_row_group_properties_inherits_base_settings() {
        let schema = Arc::new(Schema::new(vec![
            Field::new("id", DataType::Int64, false),
            Field::new("name", DataType::Utf8, true),
        ]));

        // offset_index_disabled getter returns false if page stats are enabled,
        // so we disable page stats to test the offset_index_disabled setting
        let base_props = Arc::new(
            WriterProperties::builder()
                .set_writer_version(WriterVersion::PARQUET_2_0)
                .set_compression(Compression::ZSTD(Default::default()))
                .set_data_page_size_limit(512 * 1024)
                .set_data_page_row_count_limit(5000)
        ...
        let config = AdaptiveWriterConfig {
            ...
            user_disabled_dictionary: vec![],
            user_enabled_dictionary_always: vec![],
            user_enabled_dictionary_analyze: vec![],
        ...
        let row_group_props =
            build_row_group_properties(&schema, &base_props, &config, &resolved, &analysis, 1000)
                .unwrap();

        assert_eq!(row_group_props.writer_version(), WriterVersion::PARQUET_2_0);
        assert_eq!(
            row_group_props.compression(&ColumnPath::from("")),
            Compression::ZSTD(Default::default())
        );
        assert_eq!(row_group_props.data_page_size_limit(), 512 * 1024);
        assert_eq!(row_group_props.data_page_row_count_limit(), 5000);
        assert_eq!(row_group_props.write_batch_size(), 2048);
        assert_eq!(
            row_group_props.dictionary_page_size_limit(),
            2 * 1024 * 1024
        );
        assert!(row_group_props.offset_index_disabled());
        assert!(row_group_props.coerce_types());
        assert_eq!(row_group_props.created_by(), "test-writer");
    }
}
